...
#            self.lCamSheetP_2.setPixmap(self.toPixImg(imgray))
    ...
    def bUpdateCamClick(self):
        ...
        expos=[int(self.tExposureTimeRawPsheet.toPlainText()) ]
        exposCalc=[max(35,min(int(expos[i]/35)*35,999985)) for i in range(len(expos))] 
        exposDic={"pSheet":exposCalc[0]}
...
#---basler相机初始化，拍照
    def camIni(self):
        ...
        sn=[int(self.tPsheetSn.toPlainText())]
        snStr=[str(i) for i in sn]
        print("sn:",sn)
        snDic={str(sn[0]):"pSheet"}        
