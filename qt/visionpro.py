...
def find(target):
    ...
    try:
        ...
        ##运算要返回的结果
        try:
            x=int(myCircleTool[trgDic[target]].Results.GetCircle().CenterX+myPMAlignTool[trgDic[target]].Results[0].GetPose().TranslationX)
            y=int(myCircleTool[trgDic[target]].Results.GetCircle().CenterY+myPMAlignTool[trgDic[target]].Results[0].GetPose().TranslationY)
            r=int(myCircleTool[trgDic[target]].Results.GetCircle().Radius)
            score["circle"]=1
            circle=[(x,y),r]
        except Exception as e:
            print(str(e))
