...
#ocx = threading.Thread(target=rOCXrun())
#ocx.start()  
...
#---初始化
    def __init__(self):
        ...
        global NpostionCalcFlag,PpostionCalcFlag
        global pSheetStandX,pSheetStandY,pSheetStandR,pSheetStandD
        global pPlateStandX,pPlateStandY,pPlateStandR,pPlateStandD
        global nSheetStandX,nSheetStandY,nSheetStandR,nSheetStandD
        global nPlateStandX,nPlateStandY,nPlateStandR,nPlateStandD   
        global pSheetStandXtemp,pSheetStandYtemp,pSheetStandRtemp,pSheetStandDtemp
        global pPlateStandXtemp,pPlateStandYtemp,pPlateStandRtemp,pPlateStandDtemp
        global nSheetStandXtemp,nSheetStandYtemp,nSheetStandRtemp,nSheetStandDtemp
        global nPlateStandXtemp,nPlateStandYtemp,nPlateStandRtemp,nPlateStandDtemp 
        global stationNum
...
#        rs.Open('[' + rs_name + ']', conn, 1, 3)        
        ...
        self.bUpdateStand.clicked.connect(self.bUpdateStandClick)
...
#        fd = os.open( "debug.txt", os.O_RDWR|os.O_APPEND )                   
        ...
        nSheetStandX=float(dictPara['tNsheetStandX'])
        ...
        pSheetStandXtemp=pSheetStandX=float(dictPara['tPsheetStandX'])
        pSheetStandYtemp=pSheetStandY=float(dictPara['tPsheetStandY'])
        pSheetStandRtemp=pSheetStandR=float(dictPara['tPsheetStandR'])
        pSheetStandDtemp=pSheetStandD=float(dictPara['tPsheetStandD'])
        
        pPlateStandXtemp=pPlateStandX=float(dictPara['tPplateStandX'])
        pPlateStandYtemp=pPlateStandY=float(dictPara['tPplateStandY'])
        pPlateStandRtemp=pPlateStandR=float(dictPara['tPplateStandR'])
        pPlateStandDtemp=pPlateStandD=float(dictPara['tPplateStandD'])
        
        nSheetStandXtemp=nSheetStandX=float(dictPara['tNsheetStandX'])        
        nSheetStandYtemp=nSheetStandY=float(dictPara['tNsheetStandY'])
        nSheetStandRtemp=nSheetStandR=float(dictPara['tNsheetStandR'])
        nSheetStandDtemp=nSheetStandD=float(dictPara['tNsheetStandD'])
        
        nPlateStandXtemp=nPlateStandX=float(dictPara['tNplateStandX'])
        nPlateStandYtemp=nPlateStandY=float(dictPara['tNplateStandY'])
        nPlateStandRtemp=nPlateStandR=float(dictPara['tNplateStandR'])
        nPlateStandDtemp=nPlateStandD=float(dictPara['tNplateStandD'])
        ...
        pix21=float(dictPara['tNsheetPixMM'])
        pix22=float(dictPara['tNplatePixMM'])
        pix11=float(dictPara['tPsheetPixMM'])
        pix12=float(dictPara['tPplatePixMM'])   
...
#            sv.setDaemon(True) #设
    ...
    def tcplink(self,sock, addr,nClientIP,pClientIP):
        ...
        global nPlateStandX,nPlateStandY,nPlateStandR,nPlateStandD 
        global stationNum
        global pSheetRealTimeFlag,pPlateRealTimeFlag,nSheetRealTimeFlag,nPlateRealTimeFlag
        while True:
            try:
                data = sock.recv(1024)
                #print("data:")
                if(data!=''):
                    if(addr[0]==nClientIP):
                        ...
                        if(str(nData)[2:6]=="TRGS"):
                            try:
                                inputData=str(nData)[16:-3]                       
                                stationNum=int(inputData)
                            except:
                                pass
                            time.sleep(0.5)
                            ...
                            print("centN:",outCent,lineN,stationNum) 
                        ...
                        if(str(nData)[2:4]=="XY"):
                            ...
                            if countNsheetCalc<10:
                                try:
                                    ...
                                    self.statusBar().showMessage("正极机器人网络未连接，请运行负极机器人")
                        ...
                        if(str(nData)[2:5]=="PIX"):
                            ...
                            pix21=round(math.sqrt(dx1**2+dy1**2),3)
                            pix22=round(math.sqrt(dx2**2+dy2**2),3)
                            print("pix:",pix21,pix22)
                            nData=b'' 
#                        if(str(nData)[2:4]=="ST"):
#                            print("nData",nData)
#                            inputData=str(nData)[16:-3]                       
...
#                            pass                             
                    ...
                    if(addr[0]==pClientIP):
                        ...
                        if(str(pData)[2:6]=="TRGS"):
                            try:
                                inputData=str(pData)[16:-3]                       
                                stationNum=int(inputData)
                            except Exception as e:
                                print(str(e))
                            time.sleep(0.5)
                            ...
                            print("centP:",str(centP[0]),str(centP[1]),stationNum)
                            print(str(centP[0])+","+str(centP[1]))
                            outCent=str(centP[0])+","+str(centP[1])
                            if calcFlag==0:
                                outCent=outCent+","+str(lineP)
                            try:
                        ...
                        if(str(pData)[2:5]=="PIX"):
                            ...
                            pData=b''                             
                            pass
#                        if (pSheetRealTimeFlag or pPlateRealTimeFlag or nSheetRealTimeFlag or nPlateRealTimeFlag)==0:
#                            if(str(pData)[2:4]=="ST"):
#                                print("pData",pData)
#                                inputData=str(pData)[16:-3]                       
#                                stationNum=int(inputData)                                                 
...
#                            pass                        
    ...
    def bAutoClick(self):  
        ...
        try:
            ...
            self.statusBar().showMessage("负极机器人网络未连接，请运行负极机器人")
        try:
...
#            self.bAuto.setStyleSheet("background-color: rgb(0, 255, 0);")
    ...
    def bR1runClick(self):
        global ctrMelfaRxM
        if ocx.ctrMelfaRxM.ServerLive()==False:
    ...
    def bR1resetClick(self):
        global ctrMelfaRxM,PpostionCalcFlag
        if ocx.ctrMelfaRxM.ServerLive()==False:
            b=win32api.MessageBox(0, "机器人服务器已经关闭", "警告",win32con.MB_OK) 
            return 0
        try:           
    ...
    def bR2resetClick(self):
        global ctrMelfaRxM,NpostionCalcFlag
        if ocx.ctrMelfaRxM.ServerLive()==False:
            b=win32api.MessageBox(0, "机器人服务器已经关闭", "警告",win32con.MB_OK)
            return 0
        try:           
...
#        cv2.imwrite(directory1+"/arrayBmp3.bmp",img1) 
    ...
    def bUpdateStandClick(self):  
        ...
        pSheetStandX=pSheetStandXtemp
        pSheetStandY=pSheetStandYtemp
        pSheetStandR=pSheetStandRtemp
        pSheetStandD=pSheetStandDtemp
        
        pPlateStandX=pPlateStandXtemp
        pPlateStandY=pPlateStandYtemp
        pPlateStandR=pPlateStandRtemp
        pPlateStandD=pPlateStandDtemp
        
        nSheetStandX=nSheetStandXtemp
        nSheetStandY=nSheetStandYtemp
        nSheetStandR=nSheetStandRtemp
        nSheetStandD=nSheetStandDtemp
        
        nPlateStandX=nPlateStandXtemp
        nPlateStandY=nPlateStandYtemp
        nPlateStandR=nPlateStandRtemp
        nPlateStandD=nPlateStandDtemp    
        
        self.tPsheetStandX.setText(str(pSheetStandX))
        self.tPsheetStandY.setText(str(pSheetStandY))
        self.tPsheetStandR.setText(str(pSheetStandR))
        self.tPsheetStandD.setText(str(pSheetStandD))
        
        self.tPplateStandX.setText(str(pPlateStandX))
        self.tPplateStandY.setText(str(pPlateStandY))
        self.tPplateStandR.setText(str(pPlateStandR))
        self.tPplateStandD.setText(str(pPlateStandD))
        
        self.tNsheetStandX.setText(str(nSheetStandX))
        self.tNsheetStandY.setText(str(nSheetStandY))
        self.tNsheetStandR.setText(str(nSheetStandR))
        self.tNsheetStandD.setText(str(nSheetStandD))
        
        self.tNplateStandX.setText(str(nPlateStandX))
        self.tNplateStandY.setText(str(nPlateStandY))
        self.tNplateStandR.setText(str(nPlateStandR))
        self.tNplateStandD.setText(str(nPlateStandD)) 
...
#---机器人手动界面，按钮事件      
    ...
    #---手动走位按钮1
    def getStationNum(self,robotID):
        global pSoc,nSoc
        if robotID==1:
            pSoc.send(bytes('1001' ,'utf8'))
        if robotID==2:
            nSoc.send(bytes('1001' ,'utf8'))        
        pass
    def bR1Mgo1Click(self): 
        global MposiontCodDic,stationNum
        try:
            pSoc.send(bytes(MposiontCodDic[self.tMp1.placeholderText()]+","+self.tR1Mspeed.toPlainText() ,'utf8'))  
        except:
            self.statusBar().showMessage("正极机器人网络未连接,请运行正极机器人")  
        time.sleep(0.2)
...
#---视觉调试界面按钮事件
    ...
    def bPpostionCalclick(self):
        ...
        try:
            ...
            self.statusBar().showMessage("负极机器人网络未连接，请运行负极机器人") 
        PpostionCalcFlag=1
#        myCircleTool2[2].RunParams.ExpectedCircularArc.CenterX
#        myCircleTool2[2].RunParams.ExpectedCircularArc.CenterY
#        myLineTool2[2].RunParams.ExpectedCircularArc.StartX
#        myLineTool2[2].RunParams.ExpectedCircularArc.StartY
#        myLineTool2[2].RunParams.ExpectedCircularArc.EndX
#        myLineTool2[2].RunParams.ExpectedCircularArc.EndY
        pass

    def bNpostionCalclick(self):
        global NpostionCalcFlag
        calcFlag=0
...
#            self.lCamSheetP_2.setPixmap(self.toPixImg(imgray))
    ...
    def bTrigFunc(self,sn,file,camNum,camName,visionStand):
        ...
        global countPsheetFail,countNsheetFail,countPplateFail,countNplateFail,PpostionCalcFlag,NpostionCalcFlag
...
#            pass
        ...
        if calcFlag==0:
            ...
            if (resultCircle==0 or resultLine==0):
                if camNum==0:
 ...
 #visionStand=["tPsheetStandX","tPsheetStandY","tPsheetBadValue","tPsheetStandR","tPsheetOffsetR","tPsheetStandD","tPsheetBadDegreeValue",pix11]       
        circleDefine=(abs(float(circle[0][0])-float(dictPara[visionStand[0]]))>float(dictPara[visionStand[2]]))\
        |(abs(float(circle[0][1])-float(dictPara[visionStand[1]]))>float(dictPara[visionStand[2]]))\
        |(abs(float(round(math.degrees(line[2]),2))-float(dictPara[visionStand[5]]))>float(dictPara[visionStand[6]]))\
        |(abs(float(circle[1])*float(visionStand[7])-float(dictPara[visionStand[3]]))>float(dictPara[visionStand[4]]))

        
        if (circleDefine and calcFlag==0 and resultCircle==1 and resultLine==1):
            if camNum==0:
        ...
        if (circleDefine==0 and calcFlag==0 and resultCircle==1 and resultLine==1):
            if camNum==0:
                countPsheetOK=countPsheetOK+1
                pSheetStandXtemp=circle[0][0]
                pSheetStandYtemp=circle[0][1]
                pSheetStandRtemp=round(circle[1]*pix11,3)
                pSheetStandDtemp=round(math.degrees(line[2]),2)                
            if camNum==1:
                countPplateOK=countPplateOK+1  
                pPlateStandXtemp=circle[0][0]
                pPlateStandYtemp=circle[0][1]
                pPlateStandRtemp=round(circle[1]*pix12,3)
                pPlateStandDtemp=round(math.degrees(line[2]),2)                      
            if camNum==2:
                countNsheetOK=countNsheetOK+1
                nSheetStandXtemp=circle[0][0]
                nSheetStandYtemp=circle[0][1]
                nSheetStandRtemp=round(circle[1]*pix21,3)
                nSheetStandDtemp=round(math.degrees(line[2]),2)    
                
            if camNum==3:
                countNplateOK=countNplateOK+1  
                nPlateStandXtemp=circle[0][0]
                nPlateStandYtemp=circle[0][1]
                nPlateStandRtemp=round(circle[1]*pix22,3)
                nPlateStandDtemp=round(math.degrees(line[2]),2)                    
...
#            self.outDebug("正极过渡片搜索成功")
            ...
            cv2.putText(img1,"circle:"+str((round(circle[0][0],1),round(circle[0][1],1)))+"R="+str(round(circle[1]*visionStand[7],2))+"mm offset:"+str(round(math.degrees(line[2]),2))+"deg",(10,100),cv2.FONT_HERSHEY_COMPLEX,3,(0,0,255),5)
...
#                self.outDebug("模型匹配成功")
        ...
        print("stationNum",stationNum)
        if (pSheetRealTimeFlag or pPlateRealTimeFlag or nSheetRealTimeFlag or nPlateRealTimeFlag)==0:
            access.accessWrite(stationNum,camNum,currentTime,circle[0][0],circle[0][1],round(math.degrees(line[2]),2),float(circle[1])*float(visionStand[7]),result)                
    ...
    def bPplateTrigClick(self):
        ...
        camNum=1
        camName="pPlate"
        visionStand=["tPplateStandX","tPplateStandY","tPplateBadValue","tPplateStandR","tPplateOffsetR","tPplateStandD","tPplateBadDegreeValue",pix12]
    ...
    def bNsheetTrigClick(self):
        ...
        camNum=2
        camName="nSheet"
        visionStand=["tNsheetStandX","tNsheetStandY","tNsheetBadValue","tNsheetStandR","tNsheetOffsetR","tNsheetStandD","tNsheetBadDegreeValue",pix21]
    ...
    def bNplateTrigClick(self):
        ...
        camNum=3
        camName="nPlate"
        visionStand=["tNplateStandX","tNplateStandY","tNplateBadValue","tNplateStandR","tNplateOffsetR","tNplateStandD","tNplateBadDegreeValue",pix22]
...
#---F机器人OCX控件操作功能    
    ...
    def rServer(self,robot,order):
        ...
        sSendDataOFF = "1" + "\n" +"0"
        if order==1:
            lStatus = ocx.ctrMelfaRxM.RequestServiceM(robot, 403, len(sSendDataON), sSendDataON, 0, 0, 0)
        if order==0:
            lStatus = ocx.ctrMelfaRxM.RequestServiceM(robot, 403, len(sSendDataOFF), sSendDataOFF, 0, 0, 0)
            
    def rStop(self,robot):
        sSendData = "0"
        lStatus = ocx.ctrMelfaRxM.RequestServiceM(robot, 401, len(sSendData), sSendData, 0, 0, 0)  
    
    def rReset(self,robot):
        #错误复位
        sSendData=""
        lStatus = ocx.ctrMelfaRxM.RequestServiceM(robot, 407, 0, sSendData, 0, 0, 0) 
        time.sleep(0.2)
        
        sSendData="1"
        lStatus = ocx.ctrMelfaRxM.RequestServiceM(robot, 409, len(sSendData), sSendData, 0, 0, 0)
        time.sleep(0.2)
        ...
        sSendData=""
        lStatus = ocx.ctrMelfaRxM.RequestServiceM(robot, 408, 0, sSendData, 0, 0, 0)  
        
        sSendData="1"+"\n"+"MAINSELECT"
        lStatus = ocx.ctrMelfaRxM.RequestServiceM(robot, 416, len(sSendData), sSendData, 0, 0, 0) 
    ...
    def rRunMain(self,robot):
        #修改速度
        sSendData="20"
        lStatus = ocx.ctrMelfaRxM.RequestServiceM(robot, 404, len(sSendData), sSendData, 0, 0, 0)
        time.sleep(0.5)
        sSendData="1"+"\n"+"MAINSELECT"+"\n"+"1"
        lStatus = ocx.ctrMelfaRxM.RequestServiceM(robot, 400, len(sSendData), sSendData, 0, 0, 0)         
    ...
    def rWritePosition(self,robot):        
        ...
        if robot==2:
            self.bR2run.setStyleSheet("background-color: rgb(255, 0, 0);")
        time.sleep(1)
        lStatus = ocx.ctrMelfaRxM.RequestServiceM(robot, 102, len(sSendData), sSendData, 0, 0, 0)
        flag=0
        time.sleep(0.2)
        ms=ocx.ctrMelfaRxM.CheckRecvMsg()
        for i in range(ms+1): 
            if ocx.ctrMelfaRxM.CheckRecvMsg()>0:                       
                print("ctrMelfaRxM.CheckRecvMsg()",ocx.ctrMelfaRxM.CheckRecvMsg())
                a=ocx.ctrMelfaRxM.GetRecvDataM(robot,102,sRecvData,5,lError)
                print(a)
                if "MAINSELECT" in a[3]:
                    sSendData=a[3] 
                    ocx.ctrMelfaRxM.RequestServiceM(robot, 128, len(sSendData), sSendData, 0, 0, 0)
                    flag=1  
                    return 0
        lStatus = ocx.ctrMelfaRxM.RequestServiceM(robot, 102, len(sSendData), sSendData, 0, 0, 0)
        ms=ocx.ctrMelfaRxM.CheckRecvMsg()
...
#---F QT界面刷新程序         
    def GUIfresh(self):        
        ...
        self.tPsheetTotal.setText(str(countPsheetOK+countPsheetNG+countPsheetFail))
        self.tPsheetGood.setText(str(countPsheetOK))
        self.tPsheetBad.setText(str(countPsheetNG))
        if ((countPsheetOK+countPsheetNG)>0):
            self.tPsheetGoodRate.setText(str(round(float(countPsheetOK/(countPsheetOK+countPsheetNG+countPsheetFail))*100,4)) +"%" )          
            self.tPsheetBadRate.setText(str(round(float(countPsheetNG/(countPsheetOK+countPsheetNG))*100,4))+"%"  ) 
        
        self.tPplateTotal.setText(str(countPplateOK+countPplateNG+countPplateFail))
        self.tPplateGood.setText(str(countPplateOK))
        self.tPplateBad.setText(str(countPplateNG))
        if ((countPplateOK+countPplateNG)>0):
            self.tPplateGoodRate.setText(str(round(float(countPplateOK/(countPplateOK+countPplateNG+countPplateFail)*100),4))+"%"  )          
            self.tPplateBadRate.setText(str(round(float(countPplateNG/(countPplateOK+countPplateNG+countPplateFail)*100),4))+"%"  )         
        
        self.tNsheetTotal.setText(str(countNsheetOK+countNsheetNG+countNsheetFail))
        self.tNsheetGood.setText(str(countNsheetOK))
        self.tNsheetBad.setText(str(countNsheetNG))
        if ((countNsheetOK+countNsheetNG)>0):
            self.tNsheetGoodRate.setText(str(round(float(countNsheetOK/(countNsheetOK+countNsheetNG+countNsheetFail)*100),4))+"%"  )          
            self.tNsheetBadRate.setText(str(round(float(countNsheetNG/(countNsheetOK+countNsheetNG+countNsheetFail)*100),4)) +"%" )         

        self.tNplateTotal.setText(str(countNplateOK+countNplateNG+countNplateFail))
        self.tNplateGood.setText(str(countNplateOK))
        self.tNplateBad.setText(str(countNplateNG))
        if ((countNplateOK+countNplateNG)>0):
            self.tNplateGoodRate.setText(str(round(float(countNplateOK/(countNplateOK+countNplateNG+countNplateFail)*100),4))+"%" )          
            self.tNplateBadRate.setText(str(round(float(countNplateNG/(countNplateOK+countNplateNG+countNplateFail)*100),4)) +"%" )                                           
